...
class WakeWordDetector:
    def __init__(self):
        ...
        self.rate_limiter = defaultdict(list)  # session_uid -> [timestamps]
        self.last_detections = {}  # session_uid -> last_detection_time
        
    def _load_config(self) -> Dict[str, Any]:
        """Load wake word configuration from JSON file with fallback to defaults."""
        config_path = os.environ.get("WAKE_WORD_CONFIG_PATH", "/app/config/wake_word_config.json")
        
        # Default configuration as fallback
        default_config = {
            "patterns": {
                "primary": ["hey raven", "hello raven", "hi raven"],
                "secondary": ["okay raven", "excuse me raven", "raven", "hey haven", "hello haven"],
                "conversational": ["raven can you", "raven could you", "raven will you", "raven would you", "raven please", "raven help me", "raven tell me"],
                "question_patterns": ["raven what", "raven where", "raven when", "raven who", "raven why", "raven how"],
            ...
            "extraction": {
                "default_fallback_question": "How can I help you?",
        ...
        try:
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    ...
                    logger.info(f"Loaded wake word configuration from {config_path}")
                    return config
            else:
                logger.warning(f"Wake word config file not found at {config_path}, using defaults")
                return default_config
        except Exception as e:
            logger.error(f"Error loading wake word config: {e}, using defaults")
            return default_config
    
    def _compile_patterns(self) -> List[Tuple[re.Pattern, str, float]]:
        """Compile regex patterns with their categories and thresholds."""
        compiled = []
        patterns_config = self.config["patterns"]
        sensitivity = self.config["sensitivity"]
        
        # Primary patterns (highest confidence)
        for pattern in patterns_config.get("primary", []):
            regex = rf'\b{re.escape(pattern)}\b'
            compiled.append((re.compile(regex, re.IGNORECASE), "primary", sensitivity["primary_threshold"]))
        
        # Secondary patterns (medium confidence)
        for pattern in patterns_config.get("secondary", []):
            regex = rf'\b{re.escape(pattern)}\b'
            compiled.append((re.compile(regex, re.IGNORECASE), "secondary", sensitivity["secondary_threshold"]))
        
        # Conversational patterns
        for pattern in patterns_config.get("conversational", []):
            regex = rf'\b{re.escape(pattern)}\b'
            compiled.append((re.compile(regex, re.IGNORECASE), "conversational", sensitivity["primary_threshold"]))
        
        # Question patterns
        for pattern in patterns_config.get("question_patterns", []):
            regex = rf'\b{re.escape(pattern)}\b'
            compiled.append((re.compile(regex, re.IGNORECASE), "question", sensitivity["primary_threshold"]))
        
        # Punctuation-based patterns
        for pattern in patterns_config.get("punctuation_based", []):
            regex = rf'\b{re.escape(pattern)}'
            compiled.append((re.compile(regex, re.IGNORECASE), "punctuation", sensitivity["secondary_threshold"]))
        
        logger.info(f"Compiled {len(compiled)} wake word patterns")
        return compiled
    
    def _fuzzy_match(self, text: str, target: str, threshold: float = 0.8) -> bool:
        """Check for fuzzy string matching to handle ASR errors."""
        if not self.config["sensitivity"].get("fuzzy_match_enabled", False):
            return False
            
        similarity = SequenceMatcher(None, text.lower(), target.lower()).ratio()
        return similarity >= threshold
    
    def _is_rate_limited(self, session_uid: str) -> bool:
        """Check if wake word detection should be rate limited for this session."""
        if not self.config["rate_limiting"].get("enabled", True):
            return False
            
        now = datetime.now(timezone.utc)
        cooldown = self.config["rate_limiting"].get("cooldown_seconds", 3)
        max_per_minute = self.config["rate_limiting"].get("max_detections_per_minute", 15)
        
        # Check cooldown period
        last_detection = self.last_detections.get(session_uid)
        if last_detection and (now - last_detection).total_seconds() < cooldown:
            return True
        
        # Check per-minute rate limit
        one_minute_ago = now.timestamp() - 60
        recent_detections = self.rate_limiter[session_uid]
        recent_detections = [ts for ts in recent_detections if ts > one_minute_ago]
        self.rate_limiter[session_uid] = recent_detections
        
        if len(recent_detections) >= max_per_minute:
            return True
        
        return False
    
    def _record_detection(self, session_uid: str):
        """Record a wake word detection for rate limiting."""
        now = datetime.now(timezone.utc)
        self.last_detections[session_uid] = now
        self.rate_limiter[session_uid].append(now.timestamp())
    ...
    def detect_and_extract(self, text: str, session_uid: str = "") -> Optional[str]:
        ...
        Returns:
            The extracted question if wake word is detected, None otherwise
        """
        ...
        # Check rate limiting
        if session_uid and self._is_rate_limited(session_uid):
            logger.debug(f"Wake word detection rate limited for session {session_uid}")
            return None
        ...
        for pattern, pattern_type, threshold in self.compiled_patterns:
            match = pattern.search(text_cleaned)
            if match:
                confidence = threshold  # Use threshold as base confidence
        ...
        if not best_match and self.config["sensitivity"].get("fuzzy_match_enabled", False):
            fuzzy_patterns = [
                ("hey raven", "hey haven"),
                ("hello raven", "hello haven"),
                ("hi raven", "hi haven")
            ]
            
            for correct, error_variant in fuzzy_patterns:
                if error_variant in text_cleaned.lower():
                    # Create a pseudo-match for fuzzy detection
                    start_pos = text_cleaned.lower().find(error_variant)
                    end_pos = start_pos + len(error_variant)
                    
                    class FuzzyMatch:
                        def __init__(self, start, end, text):
                            self._start, self._end, self._text = start, end, text
                        def end(self): return self._end
                        def group(self): return self._text[self._start:self._end]
                    
                    best_match = FuzzyMatch(start_pos, end_pos, text_cleaned)
                    best_confidence = self.config["sensitivity"]["secondary_threshold"]
                    best_pattern_type = "fuzzy"
                    break
        
        if best_match:
            # Extract the question part after the wake word
            wake_word_end = best_match.end()
            question = text_cleaned[wake_word_end:].strip()
            
            # Clean up the question
            question = re.sub(r'^[,\s]+', '', question)  # Remove leading commas and spaces
            question = re.sub(r'\s+', ' ', question)     # Normalize whitespace
            
            min_length = self.config["sensitivity"].get("min_question_length", 3)
            max_length = self.config["sensitivity"].get("max_question_length", 200)
            
            if question and len(question) >= min_length and len(question) <= max_length:
                logger.info(f"Wake word detected [{best_pattern_type}]: '{best_match.group()}' -> Question: '{question}' (confidence: {best_confidence:.2f})")
                
                # Record detection for rate limiting
                if session_uid:
                    self._record_detection(session_uid)
                
                return question
            else:
                logger.debug(f"Wake word detected but question too short/long or empty: '{text_cleaned}'")
                fallback = self.config["extraction"].get("default_fallback_question", "How can I help you?")
...
# Stream name for wake word commands
WAKE_WORD_STREAM_NAME = "hey_raven_commands"

def detect_wake_word_and_extract_question(text: str, session_uid: str = "") -> Optional[str]:
    """
    Enhanced wake word detection function with rate limiting and fuzzy matching.
    ...
    Returns:
        The extracted question if wake word is detected, None otherwise
    """
    return wake_word_detector.detect_and_extract(text, session_uid)

async def publish_wake_word_command(
    ...
    """
    Publish a wake word command to the Redis stream for LLM processing.
    
    Args:
        redis_c: Redis client instance
        question: The extracted question
        session_uid: Session UID from the transcription
    ...
    try:
        ...
        message_id = await redis_c.xadd(WAKE_WORD_STREAM_NAME, stream_message)
        
        logger.info(f"Published wake word command to {WAKE_WORD_STREAM_NAME}: {message_id}")
        logger.debug(f"Command data: {command_data}")
        
        return True
        
    except redis.exceptions.RedisError as e:
        logger.error(f"Failed to publish wake word command to Redis: {e}", exc_info=True)
        return False
    except Exception as e:
        logger.error(f"Unexpected error publishing wake word command: {e}", exc_info=True)
        return False
    ...
    try:
        ...
        async with async_session_local() as db:
            ...
            for i, segment in enumerate(stream_data.get('segments', [])):
                 ...
                 # Wake word detection on transcription text
                 if text_content and session_uid_from_payload:
                     detected_question = detect_wake_word_and_extract_question(text_content, session_uid_from_payload)
                     if detected_question:
                         logger.info(f"[WakeWord Detected] Msg:{message_id}/Meet:{internal_meeting_id}/Seg:{start_time_key} - Question: '{detected_question}'")
                         # Publish wake word command to LLM processor
                         try:
                             await publish_wake_word_command(
                                 redis_c=redis_c,
                                 question=detected_question,
                                 session_uid=session_uid_from_payload,
                                 meeting_id=internal_meeting_id,
                                 context=f"Meeting segment at {start_time_float}s"
                             )
                         except Exception as wake_err:
                             logger.error(f"[WakeWord Error] Failed to publish wake word command for segment {start_time_key}: {wake_err}")
                             # Don't fail the whole segment processing for wake word errors
