...
const prepareForRecording = async (page: Page): Promise<void> => {
  ...
  await page.exposeFunction("logBot", (msg: string) => {
    ...
    // --- ADDED: Setup TTS Audio Playback & Audio Enhancements ---
    await page.evaluate(() => {
      // Initialize audio session state in browser context
      (window as any).audioSessionState = {
        isPlaying: false,
      ...
      (window as any).onVoiceActivityDetected = (result: any) => {
        (window as any).audioSessionState.voiceActivityDetected = result.isVoiceDetected;
        (window as any).audioSessionState.audioLevel = result.audioLevel;
        
        // Log significant voice activity changes
        if (result.isVoiceDetected && result.confidence > 0.8) {
          (window as any).logBot?.(`[Voice Activity] Strong voice detected (confidence: ${result.confidence.toFixed(2)})`);
        }
      };

    // Setup Web Audio API
    (window as any).initializeAudioContext = () => {
      if (!(window as any).audioSessionState.audioContext) {
        try {
          (window as any).audioSessionState.audioContext = new (window.AudioContext || (window as any).webkitAudioContext)();
          (window as any).logBot?.('[Audio] Web Audio API context initialized');
          return true;
        } catch (error: any) {
          (window as any).logBot?.(`[Audio] Failed to initialize Web Audio API: ${error.message}`);
          return false;
        }
      }
      return true;
    ...
    (window as any).base64ToArrayBuffer = (base64: string) => {
    ...
    // Main TTS audio playback function
    (window as any).playTTSAudio = async (audioMessage: any) => {
      (window as any).logBot?.(`[Audio] 🎵 Starting playback for message: ${audioMessage.message_id}`);
      
      // Validate audio message
      if (!audioMessage || !audioMessage.message_id || !audioMessage.audio_data) {
        (window as any).logBot?.('[Audio] ❌ Invalid audio message - missing required fields');
        return;
      }
      
      (window as any).logBot?.(`[Audio] Audio format: ${audioMessage.audio_format}, size: ${audioMessage.audio_data.length} chars`);
      
      try {
        // Initialize audio context if needed
        if (!(window as any).initializeAudioContext()) {
          (window as any).logBot?.('[Audio] ❌ Failed to initialize audio context');
          (window as any).notifyAudioPlaybackComplete?.(audioMessage.message_id);
          return;
        }

        const audioContext = (window as any).audioSessionState.audioContext;
        
        // Decode audio data
        let audioBuffer;
        try {
          const arrayBuffer = (window as any).base64ToArrayBuffer(audioMessage.audio_data);
          audioBuffer = await audioContext.decodeAudioData(arrayBuffer);
          (window as any).logBot?.(`[Audio] Audio decoded successfully, duration: ${audioBuffer.duration}s`);
        } catch (decodeError: any) {
          (window as any).logBot?.(`[Audio] Failed to decode audio: ${decodeError.message}`);
          (window as any).notifyAudioPlaybackComplete?.(audioMessage.message_id);
          return;
        }

        // Disable microphone before playback
        await (window as any).disableMicrophone?.();
        
        // Set playing state
        (window as any).audioSessionState.isPlaying = true;
        (window as any).audioSessionState.currentAudioId = audioMessage.message_id;
        ...
        source.onended = async () => {
          (window as any).logBot?.(`[Audio] Playback completed for message: ${audioMessage.message_id}`);
          
          // Reset state
          (window as any).audioSessionState.isPlaying = false;
          (window as any).audioSessionState.currentAudioId = null;
          
          // Re-enable microphone after playback
          setTimeout(async () => {
            await (window as any).enableMicrophone?.();
            // Notify Node.js that playback is complete
            (window as any).notifyAudioPlaybackComplete?.(audioMessage.message_id);
          }, 500); // Small delay before re-enabling mic
        };

        // Start playback
        source.start();
        (window as any).logBot?.(`[Audio] Audio playback started for message: ${audioMessage.message_id}`);

      } catch (error: any) {
        (window as any).logBot?.(`[Audio] Error during audio playback: ${error.message}`);
    ...
    (window as any).disableMicrophone = async () => {
      try {
        const muteButton = document.querySelector('[aria-label*="Turn off microphone"], [aria-label*="Mute"], [aria-label*="mute"]') as HTMLElement;
        if (muteButton && !muteButton.getAttribute('aria-pressed')) {
          muteButton.click();
          (window as any).audioSessionState.microphoneEnabled = false;
          (window as any).logBot?.('[Audio] Microphone disabled for TTS playback');
          return true;
        } else if (muteButton && muteButton.getAttribute('aria-pressed') === 'true') {
          (window as any).logBot?.('[Audio] Microphone already disabled');
          return true;
        }
        (window as any).logBot?.('[Audio] Mute button not found');
        return false;
      } catch (error: any) {
        (window as any).logBot?.(`[Audio] Error disabling microphone: ${error.message}`);
        return false;
      }
    };

    (window as any).enableMicrophone = async () => {
      try {
        const unmuteButton = document.querySelector('[aria-label*="Turn on microphone"], [aria-label*="Unmute"], [aria-label*="unmute"]') as HTMLElement;
        if (unmuteButton && unmuteButton.getAttribute('aria-pressed') === 'true') {
          unmuteButton.click();
          (window as any).audioSessionState.microphoneEnabled = true;
          (window as any).logBot?.('[Audio] Microphone enabled after TTS playback');
          return true;
        } else if (unmuteButton && !unmuteButton.getAttribute('aria-pressed')) {
          (window as any).logBot?.('[Audio] Microphone already enabled');
          return true;
        }
        (window as any).logBot?.('[Audio] Unmute button not found');
        return false;
      } catch (error: any) {
        (window as any).logBot?.(`[Audio] Error enabling microphone: ${error.message}`);
        return false;
      }
    };

    (window as any).logBot?.('[Audio] TTS Audio playback system initialized');
  });
  // --- ----------------------------------------- ---
            ...
            const connectToWhisperLive = (wsUrl: string | null) => {
              ...
              try {
                ...
                socket.onopen = function () {
                  ...
                  // Notify Node.js context of the new session UID
                  if (typeof (window as any).updateWhisperLiveSessionUID === 'function') {
                    (window as any).updateWhisperLiveSessionUID(currentSessionUid);
                  } else {
                    (window as any).logBot?.('[Session] updateWhisperLiveSessionUID function not available');
                  }
